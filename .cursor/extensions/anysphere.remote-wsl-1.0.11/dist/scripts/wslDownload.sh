...
download() {
    ...
    # If REAL_COMMIT is set, use downloads.cursor.com for the URL
    if [ -n "$REAL_COMMIT" ]; then

     # if version ends with -lab, then it's lab
     # if version ends with -nightly, then its nightly
     # otherwise its production

      # Determine the release line (lab, nightly, or production)
      case "$VERSION" in
        *-lab)
          line="lab"
          ;;
      ...
      local_url="https://downloads.cursor.com/${line}/${REAL_COMMIT}/${TARGET}/${PLATFORM}/cursor-reh-${TARGET}-${PLATFORM}.tar.gz"
    else
